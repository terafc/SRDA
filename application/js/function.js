...
function downloadAllRendu(id_subject,id_creator){
	$.ajax({
		...
		success:function(html){
			$('.downloadAllRendu').hide();//On cache la div qui contiendra la réponse du serveur
			$('.downloadAllRendu').html(html);//On ajoute la réponse dans la div
			document.getElementById("uploadZip").click();//On simule un click sur le lien 
		}
	});
}
